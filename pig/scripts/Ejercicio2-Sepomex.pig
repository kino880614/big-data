  ...
  codSepomex = load '/inputData/CPdescarga.txt' USING PigStorage('|') as (d_codigo:int,d_asenta:chararray,d_tipo_asenta:chararray,d_mnpio:chararray, d_estado:chararray,	d_ciudad:chararray,	d_CP:chararray,	c_estado:int, c_oficina:int, c_CP:int, c_tipo_asenta:int, c_mnpio:int, id_asenta:int,d_zona:chararray,c_cve_ciudad:int);
